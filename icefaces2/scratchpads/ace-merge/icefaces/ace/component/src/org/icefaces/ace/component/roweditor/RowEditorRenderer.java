...
public class RowEditorRenderer extends CoreRenderer {
    ...
    public void decode(FacesContext context, UIComponent component) {
        ...
        if(params.containsKey(clientId)) {
            ...
            if (table.isTreeRootNode()) {
                treeModel = (TreeDataModel)table.getModel();
                treeModel.setRowIndex(-1);
                String editedId = params.get(table.getClientId(context) + "_editedRowId");
                if (editedId.indexOf('.') > 0) {
                    int lastSepIndex = editedId.lastIndexOf('.');
                    editedId = editedId.substring(lastSepIndex+1);
                }
                int editedRowId = Integer.parseInt(editedId);
                treeModel.setRowIndex(editedRowId);
                table.queueEvent(new RowEditEvent(table, treeModel.getRowData()));
