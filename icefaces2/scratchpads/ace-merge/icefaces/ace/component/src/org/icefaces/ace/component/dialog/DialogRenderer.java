...
public class DialogRenderer extends CoreRenderer {
    ...
    protected void encodeScript(FacesContext context, Dialog dialog) throws IOException {
        ...
        String widgetVar = this.resolveWidgetVar(dialog);
        writer.write("if (" + "typeof " + widgetVar + " !== \"undefined\"" + " && " + widgetVar + ".closeIfOpen) " + widgetVar + ".closeIfOpen(" + dialog.isVisible() + "); " + widgetVar + " = new ");
		JSONBuilder jb = JSONBuilder.create();
		jb.beginFunction("ice.ace.Dialog")
			.item(clientId)
			.beginMap()
				.entry("autoOpen", dialog.isVisible())
