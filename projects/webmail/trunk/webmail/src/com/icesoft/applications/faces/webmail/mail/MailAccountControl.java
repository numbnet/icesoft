...
/**
 * <p>
 * The <code>MailAccountControl</code> acts as a control for JavaMail related
 * tasks. The control works with a mail account and resposible for handling
 * everything from connection to the <code>MailAccount</code> to sending
 * messages.
 * </p>
 * 
 * @since 1.0
...
public class MailAccountControl {
	...
	// If the server is running Sun JDK 1.4 then we can use suns incomingSsl
	// provider,
	// com.sun.net.incomingSsl.internal.incomingSsl.Provider. However when using
	// IBM webSphere
	...
	static {
		...
		try {
			Class sunSecurityProviderClass = Class
					.forName("com.sun.net.ssl.internal.ssl.Provider");
			Constructor sunSecurityConstructor = sunSecurityProviderClass
					.getConstructor((Class[]) null);
			java.security.Provider sunSecurityProvider = (java.security.Provider) sunSecurityConstructor
			...
			if (log.isDebugEnabled())
				log
						.error("Could not class load com.sun.net.ssl.internal.ssl.Provider ");
		}
		// if the sun Security provider could not be loaded then we can assume
		// that we are using
		// an IBM JVM and thus must load there provider.
		if (!loadedSecurityProvider) {
			try {
				Class sunSecurityProviderClass = Class
						.forName("com.ibm.jsse.IBMJSSEProvider");
				Constructor sunSecurityConstructor = sunSecurityProviderClass
						.getConstructor((Class[]) null);
				java.security.Provider sunSecurityProvider = (java.security.Provider) sunSecurityConstructor
						.newInstance((Object[]) null);
				...
				if (log.isDebugEnabled())
					log
							.error("Could not class load com.ibm.jsse.IBMJSSEProvider ");
			}
		}
		// lastly if we get this far there is no SSL provider and thus the
	 ...
	 * 
	 * @return true if the pinging of mail account was sucessful; otherwise,
	 *         false.
	 */
	...
	public boolean connect() {
		try {
			...
			if (mailStore != null && mailStore.isConnected()) {
				...
				if (mailAccount.getProtocol().toLowerCase().startsWith("imap")) {
					if (mailAccount.isIncomingSsl()) {
						mailProperties
								.setProperty("mail.imaps.socketFactory.class",
										"com.icesoft.applications.faces.webmail.mail.BypassSSLSocketFactory");
						mailProperties.setProperty(
								"mail.imaps.socketFactory.fallback", "false");
						mailProperties.setProperty("mail.imaps.port", String
								.valueOf(mailAccount.getIncomingPort()));
						mailProperties
								.setProperty("mail.imaps.socketFactory.port",
										String.valueOf(mailAccount
												.getIncomingPort()));
					}
					// otherwise log on using http, avoid using incomingSsl
					// properties as
					// it will botch the connection .
					else {
						mailProperties.remove("mail.imaps.socketFactory.class");
						mailProperties
								.remove("mail.imaps.socketFactory.fallback");
						mailProperties.remove("mail.imaps.port");
						mailProperties.remove("mail.imaps.socketFactory.port");
					}
				}

				// connection url
				URLName urlName = new URLName(mailAccount.getProtocol()
						+ ((mailAccount.isIncomingSsl() && mailAccount.getProtocol().toLowerCase().startsWith("imap")) ? "s" : "") + "://"
						+ mailAccount.getIncomingHost());
				...
				// mailStore.connect();
				String userName = mailAccount.getProtocol().toLowerCase().startsWith("imap") ? mailAccount.getMailUsername() + "@" + mailAccount.getHost() : mailAccount.getMailUsername();
				int port = (mailAccount.getIncomingPort() > 0) ? mailAccount.getIncomingPort() : -1;
				mailStore.connect(mailAccount.getIncomingHost(), port, userName, mailAccount.getPassword());
				return true;
			}
			...
			log.error("Connection Error - Misc. Exception "
					+ mailAccount.toString());
			return false;
		}
	}
	 ...
	 * 
	 * @param folderName
	 *            folder name to create if not found.
	 */
	 ...
	 * 
	 * @return list of mail account folders. Null if there are no folders or if
		...
		try {
			...
			if ((rootFolder.getType() & Folder.HOLDS_FOLDERS) != 0) {
				...
				for (int i = 0; i < mailFolders.length; i++) {
					...
					tmpMailFolder.setWebmailMediator(mailAccount
							.getMailManager().getMediator());
			...
			log.error("IllegalStateException on list method "
					+ mailAccount.toString());
		} catch (NoSuchProviderException e) {
			log.error("NoSuchProviderException on list method "
					+ mailAccount.toString());
		} catch (AuthenticationFailedException e) {
			log.error("AuthenticationFailedException on list method "
					+ mailAccount.toString());
		} catch (MessagingException e) {
			log.error("Messaging Exception on list method "
					+ mailAccount.toString());
		}
	 ...
	 * 
	 * @param message
	 *            message to be used to create a pre-formated forward message
	 */
	 ...
	 * 
	 * @param message
	 *            message to be used to create a pre-formated reply message
	 */
	public MessageBean createReplyMessage(MessageBean message) {
		...
		try {
			// create simple reply
			MimeMessage replyMessage = (MimeMessage) message.getMessage()
					.reply(false);
	 ...
	 * 
	 * @param message
	 *            message to be used to create a pre-formated reply to all
	 *            message
	 */
	 ...
	 * 
	 * @param messages
	 *            messages to delete, must all be of type Message Bean or of
	 *            type javax.mail.Message.
	 * @param exponge
	 *            true to imediatly exponse delete items; otherwise, false.
	...
	public boolean deleteMessage(Object[] messages, boolean exponge) {
		...
		if (folder != null) {
			try {
				...
				// flags is going to cause event to fire and thus possible
				// renders
				...
				// set messages flag to delete, if the folder is closed or
				// exponged
				...
				for (int i = messages.length - 1; i >= 0; i--) {
					...
					if (messages[i] != null
							...
							&& !((MessageBean) messages[i]).getMessage()
									.isExpunged()) {
			...
			try {
				...
				if (mailAccount.isOutgoingVerification()) {
					transport.connect(mailAccount.getOutgoingHost(),
							mailAccount.getOutgoingPort(), mailAccount
									.getMailUsername(), mailAccount
									.getPassword());
				} else {
					transport.connect(mailAccount.getOutgoingHost(),
							mailAccount.getOutgoingPort(), null, null);
				}

				// try and send the message
				if (transport.isConnected() && message.getSender() != null
					...
					// send the message.
					transport.sendMessage(message.getMessage(), message
	...
	/**
	 * Saves the messsage specified by <code>message</code>. Mime message are
	 * read-only and thus a copy must be made to re-save a message. An
	 * applications should only limit this feature to the drafts folder for
	 * read-only messages.
	 * 
	 * @param message
	 *            message to save contents of.
		...
		if (message.getMessage() != null) {
			try {
				...
				// a copy in order to save it.
					...
					if (log.isDebugEnabled()) {
						log
								.debug("Message could not be saved, read only Mime Message ");
					}
					return false;
				}
	...
	/**
	 * Copies the specified messageBean returning a copy of the origional
	 * messageBean. This should only be called when a users needed to edit a
	 * mime messageBean.
	 * 
	 * @param messageBean
	 *            messageBean to copy
	...
	public MessageBean copyMessage(MessageBean messageBean) {
		...
		try {
			...
			copyMessageBean.setMessageSenderAccount(messageBean
			...
			for (int i = 0, max = recipients.size(); i < max; i++) {
				...
				copiedRecipients.add(i, new TableEntry(tmpTableEntry.getType(),
	 ...
	 */
	public boolean moveMessages(Object[] messages, String fromFolderName,
	...
	/**
	 * Move the specified messages from this folder, to the specified
	 * destination.
	 * 
	 * @param messages
	 *            the array of message objects
	 * @param fromFolder
	 *            he folder to copy the messages from.
	 * @param destFolder
	 *            he folder to copy the messages to.
	 */
	public boolean moveMessages(Object[] messages, MailFolderBean fromFolder,
			MailFolderBean destFolder) {

		try {
			// Return if no folder was found
			if ((fromFolder == null && destFolder == null)
					|| (fromFolder != null && fromFolder.getFolder() != null && !fromFolder
							.getFolder().exists())
					|| (fromFolder != null && fromFolder.getFolder() != null && !fromFolder
							.getFolder().exists())) {
				if (log.isDebugEnabled()) {
			...
			if (destFolder != null && fromFolder != null) {
				...
				// Open the from folder if needed, destination is not needed by
				// copyMessages
				...
				// remove the message from the folder bean, the following mail
				// api
				...
				fromFolder.getFolder().copyMessages(rawMessages,
						destFolder.getFolder());

				// marke the current as deleteable.
				fromFolder.getFolder().setFlags(rawMessages,
				...
				log
						.debug("Can not move messages in their current state, null folder state ");
			}
			...
			log.error("Error Moving multiple messages failed to "
	 ...
	 * 
	 * @param message
	 *            an unsaved message object, a new message.
	 * @param destfolder
	 *            the folder wher the new message is to be saved.
	 */
	public boolean moveMessage(MessageBean message, MailFolderBean destfolder) {
		...
		if (destfolder == null) {
			if (log.isDebugEnabled()) {
				log
						.debug("Move error - Null destination folder, now where to move");
			}
			return false;
		}
		...
		try {
			...
			if (folder != null) {
				...
				folder.copyMessages(new javax.mail.Message[] { message
						.getMessage() }, destfolder.getFolder());
				// marke the current as deleteable.
				folder.setFlags(
						new javax.mail.Message[] { message.getMessage() },
						new Flags(Flags.Flag.DELETED), true);
			...
			// if the message has no folder then we can append it to the
			// destination
			else {
				if (log.isDebugEnabled())
