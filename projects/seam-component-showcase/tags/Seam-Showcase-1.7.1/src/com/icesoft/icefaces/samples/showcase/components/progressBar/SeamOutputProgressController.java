...
import org.icefaces.application.showcase.view.bean.examples.component.progressBar.OutputProgressModel;
...
@Name("outputProgressController")
public class SeamOutputProgressController implements Renderable,Serializable  {
	  ...
	  @Logger Log log;
	    ...
	    private PersistentFacesState persistentFacesState= PersistentFacesState.getInstance();
	    private String sessionId;

	    // Model where we store the dynamic properties associated with outputProgress
	    private SeamOutputProgressModel outputProgressModel;
	    ...
	    @Create
	    public void init() {
	    	log.info("create a new model");
	        outputProgressModel = new SeamOutputProgressModel();
	    ...
	    public void startLongProcress(ActionEvent event) {
	        ...
	        longRunningTaskThreadPool.execute(
	                    new LongOperationRunner(outputProgressModel,renderManager,
	        ...
	        public void run() {
	            ...
	            try {
	                for (int i = 0; i <= 100; i += 10) {
	                    ...
	                    try {
	                    		rManager.getOnDemandRenderer(sessionId).requestRender();
