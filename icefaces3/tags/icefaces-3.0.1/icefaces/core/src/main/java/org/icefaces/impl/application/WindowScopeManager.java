...
public class WindowScopeManager extends SessionAwareResourceHandlerWrapper {
	public static final String SessionSynchronizationMonitor = WindowScopeManager.class
			.getName() + "$SessionSynchronizationMonitor";
	public static final String ScopeName = "window";
	private static final Logger log = Logger.getLogger(WindowScopeManager.class
			.getName());
	private static final String seed = Integer.toString(
			new Random().nextInt(1000), 36);
	...
	public void handleSessionAwareResourceRequest(FacesContext facesContext)
			throws IOException {
	...
	public boolean isSessionAwareResourceRequest(FacesContext facesContext) {
		...
		if (isDisposeWindowRequest(parameters)) {
			// force the running of the JSF lifecycle so that the registered
			// phase listener has a chance to destroy
	...
	public static ScopeMap lookupWindowScope(FacesContext context) {
		String id = lookupAssociatedWindowID(context.getExternalContext()
				.getRequestMap());
	...
	public static String determineWindowID(FacesContext context) {
		...
		// ICE-7749: By default, in portals, the ExternalContext.getSessionMap()
		// returns a map of
		// values that are in PortletSession.PORTLET_SCOPE. But for the
		// synchronization monitor we
		// are using here, it's initially put into
		// PortletSession.APPLICATION_SCOPE so we need to
		// access it using our own session proxy which, with portlets, defaults
		// to APPLICATION_SCOPE.
		HttpSession safeSession = EnvUtils.getSafeSession(context);
		Object synchronizationMonitor = safeSession
				.getAttribute(SessionSynchronizationMonitor);
		if (synchronizationMonitor == null) {
			log.log(Level.FINE,
					"synchronization monitor not set by session listener");
			synchronizationMonitor = new SynchronizationMonitorObject();
			safeSession.setAttribute(SessionSynchronizationMonitor,
		...
		synchronized (synchronizationMonitor) {
			...
			String id = externalContext.getRequestParameterMap().get(
					"ice.window");
			try {
				for (Object scopeMap : new ArrayList(
						state.windowScopedMaps.values())) {
			...
			try {
				for (Object scopeMap : new ArrayList(
						state.disposedWindowScopedMaps)) {
					ScopeMap map = (ScopeMap) scopeMap;
					if (!map.getId().equals(id)) {
			...
			if (id == null) {
				...
				if (scopeMap == null) {
					if (state.disposedWindowScopedMaps.isEmpty()) {
						...
						scopeMap = (ScopeMap) state.disposedWindowScopedMaps
								.removeFirst();
					}
				...
				if (state.windowScopedMaps.containsKey(id)) {
					...
					// this must be a postback request while the window is
					// reloading or redirecting
					for (Object disposedScopeMap : new ArrayList(
							state.disposedWindowScopedMaps)) {
					...
					// most probably a postback received after the server or
	...
	public static void disposeWindows(final HttpSession session) {
		// The strategy is to invoke @PreDestroy on as many applicable window
		// scoped beans as possible and not to bail
		// out on the first fail.
		State state = (State) session.getAttribute(WindowScopeManager.class
				.getName());
	...
	public static synchronized void disposeWindow(final FacesContext context,
			String id, Timer timer) {
		...
		// it's possible to have dispose-window request arriving after an
		// application restart or re-deploy
		if (scopeMap != null) {
		...
		if (state.windowScopedMaps.isEmpty()) {
			long windowScopeExpiration = EnvUtils
					.getWindowScopeExpiration(context);
			// copy session beans before they're cleared on
			// FacesContext.release()
			final Map session = new HashMap(context.getExternalContext()
					.getSessionMap());
			timer.schedule(new AllWindowsClosedNotifier(state, session),
					windowScopeExpiration * 2);
		}
	...
	private static void disposeViewScopeBeans(FacesContext facesContext) {
		...
		facesContext.setExceptionHandler(new DiscardingExceptionHandler(
				oldHandler));
		...
		while (keys.hasNext()) {
			...
			if (object.getClass().isAnnotationPresent(WindowDisposed.class)) {
				...
				if (log.isLoggable(Level.FINE)) {
					log.log(Level.FINE,
							"Closing window disposed ViewScoped bean " + key);
				}
			}
		}
	...
	private static void callAnnotatedMethod(Object object, Class annotation) {
		...
		try {
			...
			log.log(Level.WARNING, "Failed to invoke" + annotation + " on "
					+ theClass, e);
		}
	}

	public static void sessionCreated(HttpSession session) {
		session.setAttribute(SessionSynchronizationMonitor,
				new SynchronizationMonitorObject());
	}
	...
	public static class ScopeMap extends HashMap {
		...
		public ScopeMap(FacesContext facesContext) {
			...
			try {
				...
				facesContext.getApplication().publishEvent(facesContext,
						PostConstructCustomScopeEvent.class, context);
			} finally {
		...
		private void disactivateIfUnused(FacesContext facesContext) {
			if (!EnvUtils.containsBeans(this)) {
				// the map *does not* contain objects (most probably beans)
				// other than the ones inserted by the framework
		...
		private void discardIfExpired(FacesContext facesContext) {
			...
			if (System.currentTimeMillis() > (deactivateTimestamp + state.expirationPeriod)) {
				...
				try {
					...
					ScopeContext context = new ScopeContext(ScopeName, this);
					facesContext.getApplication().publishEvent(facesContext,
							PreDestroyCustomScopeEvent.class, context);
				} finally {
		...
		private void disactivate(State state) {
			...
			state.disposedWindowScopedMaps.addLast(state.windowScopedMaps
					.remove(id));
		}
	}
	...
	private static State getState(FacesContext context) {
		...
		if (session != null) {
			return EnvUtils.instanceofPortletSession(session) ? getPortletState(
					context, session) : getServletState(context, session);
		} else {
			return null;
		}
	}
	...
	private static State getServletState(FacesContext context, Object session) {
		...
		State state = (State) servletSession
				.getAttribute(WindowScopeManager.class.getName());
		if (state == null) {
			state = new State(EnvUtils.getWindowScopeExpiration(context));
			servletSession.setAttribute(WindowScopeManager.class.getName(),
					state);
		}
		return state;
	}

	// portlet session is accessed in a separate method to avoid ClassNotFound
	// errors in servlet environment
	private static State getPortletState(FacesContext context, Object session) {
		...
		State state = (State) portletSession.getAttribute(
				WindowScopeManager.class.getName(),
				javax.portlet.PortletSession.APPLICATION_SCOPE);
		if (state == null) {
			state = new State(EnvUtils.getWindowScopeExpiration(context));
			portletSession.setAttribute(WindowScopeManager.class.getName(),
					state, javax.portlet.PortletSession.APPLICATION_SCOPE);
		}
		return state;
	}
	...
	private static class TimeBasedHeuristicWindowScopeSharing implements
			SharedMapLookupStrategy {
		...
		public ScopeMap lookup(FacesContext context) {
			...
			while (i.hasNext()) {
				...
				if (sm.activateTimestamp + sameWindowMaxDelay > System
						.currentTimeMillis()) {
					return sm;
				}
			}
	...
	private static class LiferayOriginalRequestWindowScopeSharing implements
			SharedMapLookupStrategy {
		private Class PortalUtilClass;
		private Method GetHttpServletRequest;
		private Method GetOriginalServletRequest;

		private LiferayOriginalRequestWindowScopeSharing()
				throws ClassNotFoundException, NoSuchMethodException {
			PortalUtilClass = Class
					.forName("com.liferay.portal.util.PortalUtil");
			GetHttpServletRequest = PortalUtilClass
					.getDeclaredMethod("getHttpServletRequest",
							javax.portlet.PortletRequest.class);
			GetOriginalServletRequest = PortalUtilClass.getDeclaredMethod(
					"getOriginalServletRequest", HttpServletRequest.class);
		}
		...
		public ScopeMap lookup(FacesContext context) {
			...
			String sharedWindowID = (String) originalRequest
					.getAttribute(WindowScopeManager.class.getName());
			return sharedWindowID == null ? null
					: (ScopeMap) state.windowScopedMaps.get(sharedWindowID);
		}

		public void associate(FacesContext context, String windowID) {
			ExternalContext externalContext = context.getExternalContext();
			HttpServletRequest originalRequest = getOriginalServletRequest(externalContext);
			originalRequest.setAttribute(WindowScopeManager.class.getName(),
					windowID);
		...
		private HttpServletRequest getOriginalServletRequest(
			...
			try {
				javax.portlet.PortletRequest portletRequest = (javax.portlet.PortletRequest) externalContext
						.getRequest();
				HttpServletRequest httpPortletRequest = (HttpServletRequest) GetHttpServletRequest
						.invoke(PortalUtilClass, portletRequest);
				HttpServletRequest originalRequest = (HttpServletRequest) GetOriginalServletRequest
						.invoke(PortalUtilClass, httpPortletRequest);
	...
	static class DiscardingExceptionHandler extends ExceptionHandlerWrapper {
		...
		public void processEvent(SystemEvent exceptionQueuedEvent) {
			...
			if (log.isLoggable(Level.FINE)) {
				log.log(Level.FINE, "Exception during window disposal "
						+ throwable);
			}
		}
	...
	public static class DetermineOrDisposeScope implements PhaseListener {
		...
		public DetermineOrDisposeScope() {
			...
			// shutdown timer
			application.subscribeToEvent(PreDestroyApplicationEvent.class,
		...
		public void afterPhase(final PhaseEvent event) {
			...
			Map parameters = externalContext.getRequestParameterMap();
			if (isDisposeWindowRequest(parameters)) {
				// shortcut the lifecycle to avoid running it with certain parts
				...
				if (EnvUtils.isICEpushPresent()) {
					try {
						...
						for (int i = 0; i < viewIDs.length; i++) {
							SessionViewManager.removeView(facesContext,
									viewIDs[i]);
						}
					} catch (RuntimeException e) {
						// missing ice.view parameters means that none of the
						// views within the page
						// was registered with PushRenderer before page unload
						log.log(Level.FINE, "Exception during dispose-window ",
								e);
					}
				}
			}
			...
			try {
				...
				if (session != null) {
					if (EnvUtils.instanceofPortletSession(session)) {
						javax.portlet.PortletSession portletSession = (javax.portlet.PortletSession) session;
						Object state = portletSession.getAttribute(
								WindowScopeManager.class.getName(),
								javax.portlet.PortletSession.APPLICATION_SCOPE);
						if (state != null) {
							portletSession
						...
						HttpSession servletSession = (HttpSession) session;
						Object state = servletSession
								.getAttribute(WindowScopeManager.class
										.getName());
						if (state != null) {
							servletSession.setAttribute(
									WindowScopeManager.class.getName(), state);
						}
					}
				}
			} catch (Exception e) {
	...
	private static class AllWindowsClosedNotifier extends TimerTask {
		...
		public void run() {
			...
			if (state.windowScopedMaps.isEmpty()) {
				...
				Iterator disposedMaps = state.disposedWindowScopedMaps
						.iterator();
