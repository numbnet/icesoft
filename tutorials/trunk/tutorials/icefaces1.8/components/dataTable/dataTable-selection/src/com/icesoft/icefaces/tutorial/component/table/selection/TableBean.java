package com.icesoft.icefaces.tutorial.component.table.selection;
...
public class TableBean {
    ...
    public void rowSelection(RowSelectorEvent event) {
        selectedRows.clear();
        for (int i=carInventory.length-1; i >=0; i--){
            if (carInventory[i].getSelected().booleanValue()) {
                selectedRows.add(carInventory[i]);
            }
        }
    }
    ...
    public void selectionChanged(ValueChangeEvent event) {
        selectedRows.clear();
        for(int i=carInventory.length-1; i>=0; i--){
            carInventory[i].setSelected(Boolean.FALSE);
        }
    }    
