...
public class ChatServiceStatelessEJB implements ChatServiceLocal {
    ...
    public UserChatSession loginToChatRoom(String chatRoom, String userName, String password) {
        ...
        UserChatSession newSession = null;
        if( user != null ){
          ...
          if( room != null ){
        	  newSession = new UserChatSession();
        	  newSession.setLive(true);
        	  newSession.setRoom(room);
        	  newSession.setUser(user);
              room.getUserChatSessions().add(newSession);
    ...
    public User login(String userName, String password)  throws LoginFailedException {
        List<User> results = em.createQuery("select object(o) from User as o where o.userName = :userName and o.password = :password")
