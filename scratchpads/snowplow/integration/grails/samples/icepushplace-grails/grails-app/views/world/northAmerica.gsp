...
<!--    <g:if test="${!regions.northAmerica}">
    <tr>
        <td colspan="6">Empty</td>
    </tr>
    </g:if>
    <g:if test="${regions.northAmerica}">
    <g:each var="elem" in="${regions.northAmerica}" status="row">
    <tr>
        <td>${elem.nickname}</td>
        <td>${elem.mood}</td>
        <td>${elem.comment}</td>
        ...
        <td>
            ...
            <input type="submit"
                   value="Post"
                   onclick="click_messageOut('${elem.region}',${row.index},'<%=session["person"].nickname%>');return false;"/>
            </form>
        </td>
    </tr>
    </g:each>
