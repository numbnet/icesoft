...
(function() {
    function setupWindowProperties() {
        ...
        if (!('width' in document)) Object.defineProperty(document, 'width', {
            get: function () {
                return Math.max(html.clientWidth, html.scrollWidth, body.scrollWidth);
            }
        });

        if (!('height' in document)) Object.defineProperty(document, 'height', {
            get: function () {
                return Math.max(html.clientHeight, html.scrollHeight, body.scrollHeight);
            }
        });
    ...
    if (document.addEventListener){
        document.addEventListener('load', setupWindowProperties, false);
    } else {
        window.attachEvent('onload', setupWindowProperties);
    }
}());
...
ice.mobi.calcFittedHeight = function(elem){
    ...
    if( window.getComputedStyle ){
        ...
        fittedHeight -= parseInt(computedStyle['border-top-width']);
        fittedHeight -= parseInt(computedStyle['border-bottom-width']);
        fittedHeight -= parseInt(computedStyle['padding-top']);
        fittedHeight -= parseInt(computedStyle['padding-bottom']);
    }
...
(function(im) {
    ...
    function DataView(clientId, cfg) {
        ...
        function initTableAlignment() {
            ...
            }, config.delay) /* hiding instantly broke scrolling when init'ing the first time on landscape ipad */
        }
...
mobi.flipswitch = {
        ...
        init: function(clientId, cfg){
            ...
            if (this.flipperEl){
                ...
                if (oldClass.indexOf('mobi-flipswitch-off')>0){
    ...
    function Accordion(clientId, cfgIn) {
        ...
        function calcCurrentHeight(){
            ...
            if (height){
                ...
                var rootRect = getRootElem().getBoundingClientRect();
                currentHeight = rootRect.height - (getContainerElem().children.length * handleHeight);
                if( currentHeight <= 0  ){
                    currentHeight = null;
                }
            }
        ...
        function getPane(id){
            var containerElem = getContainerElem();
            if( !containerElem ){
                return null;
            }
        ...
        function calcHandleHeight(){
            var handleNode = getContainerElem().querySelector('.mobi-accordion .handle');
        ...
        return {
            ...
            updateHeight: function(){
                ...
                if( openId ){
                    ...
                    if( openPaneScrollPos ){
                        openPaneContent.scrollTop = openPaneScrollPos;
                    }
                }
            } ,
            updateProperties: function (clientId, cfgUpd) {
                init(clientId, cfgUpd);  
            },
