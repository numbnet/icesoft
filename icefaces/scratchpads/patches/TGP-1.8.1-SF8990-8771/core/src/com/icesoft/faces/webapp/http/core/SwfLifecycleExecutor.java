...
public class SwfLifecycleExecutor extends LifecycleExecutor  {
	...
	private static final String SPRING_SESSION_KEY = "guaranteedUniqueA57VToken";
    ...
    public void apply(FacesContext facesContext)  {
        ...
        Map sessionObjects = ec.getSessionMap();
        ...
        Object sessionToken = sessionObjects.get( SPRING_SESSION_KEY );
        if (sessionToken == null) {
            sessionToken = new Object();
            log.debug("Generating unique object key: " + sessionToken.hashCode() + " for user: " + sessionId);
            System.out.println("Generating unique object key: " + sessionToken.hashCode() + " for user: " + sessionId);
            sessionObjects.put( SPRING_SESSION_KEY, sessionToken );
        }

        Object cc = sessionObjects.get("webflowConversationContainer");

        System.out.println("--> Looking for ConversationContainer in session finds: " + cc);

        try {

            Method m = cc.getClass().getMethod("size", new Class[] {} );
            m.setAccessible(true);
            if (m != null) {
                Object sizeValue = m.invoke( cc, new Object[] { null } );
                System.out.println("Size of conversation list is: " + sizeValue);
            }
        } catch (Exception e)  {
            System.out.println("Exception in introspective lookup: " + e);
            e.printStackTrace();
        }

        String executorHash = (flowExecutor != null) ? Integer.toString(flowExecutor.hashCode()) : "NULL";
        log.info("User: " + sessionId + ", SwfLifecycleExecutor: " + this.hashCode() + " has flowid: " + flowId + ", FlowExecutor: "
                           + executorHash +  ", thread: " + Thread.currentThread().getName());
        System.out.println("User: " + sessionId + ", SwfLifecycleExecutor: " + this.hashCode() + " has flowid: " + flowId + ", FlowExecutor: "
                           + executorHash +  ", thread: " + Thread.currentThread().getName());
        try {
            ...
            if (null != flowExecutionKey)  {
                ...
                log.info(" ----- >>>>>>> INBOUND " + sessionId + " == " + flowExecutionKey + " -------------");
                System.out.println(" ----- >>>>>>> INBOUND " + sessionId + " == " + flowExecutionKey + " -------------");
                    result = flowExecutor.resumeExecution(
                            flowExecutionKey, servletExternalContext);
                log.info(" ----- <<<<<<< OUTBOUND " + sessionId + " == " + flowExecutionKey + " -------------");
                    System.out.println(" ----- <<<<<<< OUTBOUND " + sessionId + " == " + flowExecutionKey + " -------------");
                }
            } else {
            ...
            log.warn("No flow definition found for key: " + flowExecutionKey  );
